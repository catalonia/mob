...
#ifndef DEF_ENUNCIATENS0TSLocationSearchCitiesObj_M
#define DEF_ENUNCIATENS0TSLocationSearchCitiesObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSLocationSearchCitiesObj
...
{
  [newNeighborhoodId retain];
  [_neighborhoodId release];
  _neighborhoodId = newNeighborhoodId;
}
...
{
  [newNeighborhoodName retain];
  [_neighborhoodName release];
  _neighborhoodName = newNeighborhoodName;
}
...
- (void) dealloc
{
  ...
  [self setNeighborhoodId: nil];
  [self setNeighborhoodName: nil];
  [self setState: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSLocationSearchCitiesObj *_eNUNCIATENS0TSLocationSearchCitiesObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSLocationSearchCitiesObj = (ENUNCIATENS0TSLocationSearchCitiesObj *) [ENUNCIATENS0TSLocationSearchCitiesObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSLocationSearchCitiesObj;
}
...
@end /* implementation ENUNCIATENS0TSLocationSearchCitiesObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSLocationSearchCitiesObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSLocationSearchCitiesObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSLocationSearchCitiesObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSLocationSearchCitiesObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSLocationSearchCitiesObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSLocationSearchCitiesObj *_eNUNCIATENS0TSLocationSearchCitiesObj = [[ENUNCIATENS0TSLocationSearchCitiesObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSLocationSearchCitiesObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSLocationSearchCitiesObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSLocationSearchCitiesObj autorelease];
  return _eNUNCIATENS0TSLocationSearchCitiesObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSLocationSearchCitiesObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSLocationSearchCitiesObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSLocationSearchCitiesObj from an XML reader. The element to be read is
 * "locationsearchcities".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSLocationSearchCitiesObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSLocationSearchCitiesObj *_tSLocationSearchCitiesObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element locationsearchcities."];
    }
  }

  if (xmlStrcmp(BAD_CAST "locationsearchcities", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}locationsearchcities.");
#endif
    _tSLocationSearchCitiesObj = (ENUNCIATENS0TSLocationSearchCitiesObj *)[ENUNCIATENS0TSLocationSearchCitiesObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}locationsearchcities.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSLocationSearchCitiesObj. Expected element locationsearchcities. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSLocationSearchCitiesObj. Expected element locationsearchcities. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSLocationSearchCitiesObj to XML under element name "locationsearchcities".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSLocationSearchCitiesObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "locationsearchcities", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}locationsearchcities. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSLocationSearchCitiesObj for root element {}locationsearchcities...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSLocationSearchCitiesObj for root element {}locationsearchcities...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}locationsearchcities. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}cityName of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}cityName of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setCityName: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}neighborhoodId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}neighborhoodId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setNeighborhoodId: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}neighborhoodName of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}neighborhoodName of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setNeighborhoodName: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self cityName]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}cityName."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}cityName...");
#endif
    [[self cityName] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}cityName...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}cityName."];
    }
  }
  if ([self neighborhoodId]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "neighborhoodId", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}neighborhoodId."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}neighborhoodId...");
#endif
    [[self neighborhoodId] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}neighborhoodId...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}neighborhoodId."];
    }
  }
  if ([self neighborhoodName]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "neighborhoodName", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}neighborhoodName."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}neighborhoodName...");
#endif
    [[self neighborhoodName] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}neighborhoodName...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}neighborhoodName."];
    }
  }
  if ([self state]) {
...
@end /* implementation ENUNCIATENS0TSLocationSearchCitiesObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSLocationSearchCitiesObj_M */
...
#ifndef DEF_ENUNCIATENS0TSNotifRecoReplyObj_M
#define DEF_ENUNCIATENS0TSNotifRecoReplyObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifRecoReplyObj
...
{
  [newRepliedDatetime retain];
  [_repliedDatetime release];
  _repliedDatetime = newRepliedDatetime;
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifRecoReplyObj *_eNUNCIATENS0TSNotifRecoReplyObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifRecoReplyObj = (ENUNCIATENS0TSNotifRecoReplyObj *) [ENUNCIATENS0TSNotifRecoReplyObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifRecoReplyObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifRecoReplyObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifRecoReplyObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifRecoReplyObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifRecoReplyObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifRecoReplyObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifRecoReplyObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifRecoReplyObj *_eNUNCIATENS0TSNotifRecoReplyObj = [[ENUNCIATENS0TSNotifRecoReplyObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifRecoReplyObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifRecoReplyObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifRecoReplyObj autorelease];
  return _eNUNCIATENS0TSNotifRecoReplyObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifRecoReplyObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifRecoReplyObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifRecoReplyObj from an XML reader. The element to be read is
 * "notifrecoreply".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifRecoReplyObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifRecoReplyObj *_tSNotifRecoReplyObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notifrecoreply."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notifrecoreply", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notifrecoreply.");
#endif
    _tSNotifRecoReplyObj = (ENUNCIATENS0TSNotifRecoReplyObj *)[ENUNCIATENS0TSNotifRecoReplyObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notifrecoreply.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecoReplyObj. Expected element notifrecoreply. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecoReplyObj. Expected element notifrecoreply. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifRecoReplyObj to XML under element name "notifrecoreply".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifRecoReplyObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notifrecoreply", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notifrecoreply. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifRecoReplyObj for root element {}notifrecoreply...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifRecoReplyObj for root element {}notifrecoreply...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notifrecoreply. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}repliedDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}repliedDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRepliedDatetime: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}viewed of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}viewed of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setViewed: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self repliedDatetime]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "repliedDatetime", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}repliedDatetime."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}repliedDatetime...");
#endif
    [[self repliedDatetime] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}repliedDatetime...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}repliedDatetime."];
    }
  }
  if ([self viewed]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}viewed."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}viewed...");
#endif
    [[self viewed] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}viewed...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}viewed."];
    }
  }
}
@end /* implementation ENUNCIATENS0TSNotifRecoReplyObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifRecoReplyObj_M */
...
#ifndef DEF_ENUNCIATENS0TSRecoNotificationBaseObj_M
#define DEF_ENUNCIATENS0TSRecoNotificationBaseObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRecoNotificationBaseObj
...
{
  [newRecoNotificationType retain];
  [_recoNotificationType release];
  _recoNotificationType = newRecoNotificationType;
}
...
{
  [newDatetimeBase retain];
  [_datetimeBase release];
  _datetimeBase = newDatetimeBase;
}
...
@end /* implementation ENUNCIATENS0TSRecoNotificationBaseObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRecoNotificationBaseObj (JAXB) <JAXBReading, JAXBWriting, JAXBType>

@end /*interface ENUNCIATENS0TSRecoNotificationBaseObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRecoNotificationBaseObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRecoNotificationBaseObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRecoNotificationBaseObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRecoNotificationBaseObj *_eNUNCIATENS0TSRecoNotificationBaseObj = [[ENUNCIATENS0TSRecoNotificationBaseObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRecoNotificationBaseObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRecoNotificationBaseObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRecoNotificationBaseObj autorelease];
  return _eNUNCIATENS0TSRecoNotificationBaseObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRecoNotificationBaseObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRecoNotificationBaseObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recoNotificationType of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recoNotificationType of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRecoNotificationType: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}datetimeBase of type {http://www.w3.org/2001/XMLSchema}dateTime.");
#endif
    __child = [NSDate readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}datetimeBase of type {http://www.w3.org/2001/XMLSchema}dateTime.");
#endif

    [self setDatetimeBase: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}idBase of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}idBase of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setIdBase: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self recoNotificationType]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recoNotificationType."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recoNotificationType...");
#endif
    [[self recoNotificationType] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recoNotificationType...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recoNotificationType."];
    }
  }
  if ([self datetimeBase]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "datetimeBase", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}datetimeBase."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}datetimeBase...");
#endif
    [[self datetimeBase] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}datetimeBase...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}datetimeBase."];
    }
  }
  if ([self idBase]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}idBase."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}idBase...");
#endif
    [[self idBase] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}idBase...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}idBase."];
    }
  }
}
@end /* implementation ENUNCIATENS0TSRecoNotificationBaseObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRecoNotificationBaseObj_M */
...
{
  return _restaurantExtendInfoObj;
}
...
{
  [newRestaurantExtendInfoObj retain];
  [_restaurantExtendInfoObj release];
  _restaurantExtendInfoObj = newRestaurantExtendInfoObj;
}
...
{
  [newRestaurantBuzz retain];
  [_restaurantBuzz release];
  _restaurantBuzz = newRestaurantBuzz;
}

- (void) dealloc
{
  [self setRestaurantExtendInfoObj: nil];
...
- (BOOL) readJAXBChildElement: (xmlTextReaderPtr) reader
{
  ...
  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "restaurantExtendInfoObj", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {

#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}restaurantExtendInfoObj of type {}TSRestaurantExtendInfoObj.");
#endif
    __child = [ENUNCIATENS0TSRestaurantExtendInfoObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}restaurantExtendInfoObj of type {}TSRestaurantExtendInfoObj.");
#endif

    [self setRestaurantExtendInfoObj: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}photoList of type {}TSRestaurantPhotoObj.");
#endif

    __child = [ENUNCIATENS0TSRestaurantPhotoObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}photoList of type {}TSRestaurantPhotoObj.");
#endif
  ...
  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "restaurantBuzz", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {

#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}restaurantBuzz of type {}TSRestaurantBuzzObj.");
#endif
    __child = [ENUNCIATENS0TSRestaurantBuzzObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}restaurantBuzz of type {}TSRestaurantBuzzObj.");
#endif

    [self setRestaurantBuzz: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self restaurantExtendInfoObj]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantExtendInfoObj", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}restaurantExtendInfoObj."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}restaurantExtendInfoObj...");
#endif
    [[self restaurantExtendInfoObj] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}restaurantExtendInfoObj...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}restaurantExtendInfoObj."];
    }
  }
  if ([self openNowFlag]) {
  ...
  if ([self restaurantBuzz]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantBuzz", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}restaurantBuzz."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}restaurantBuzz...");
#endif
    [[self restaurantBuzz] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}restaurantBuzz...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}restaurantBuzz."];
    }
  }
}
...
#ifndef DEF_ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj_M
#define DEF_ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj
...
{
  return _friendsNonTsUserFacebookId;
}
...
{
  [newFriendsNonTsUserFacebookId retain];
  [_friendsNonTsUserFacebookId release];
  _friendsNonTsUserFacebookId = newFriendsNonTsUserFacebookId;
}

- (void) dealloc
{
  [self setQuestionId: nil];
  [self setFriendsNonTsUserFacebookId: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj *_eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj = (ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj *) [ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj *_eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj = [[ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj autorelease];
  return _eNUNCIATENS0TSRestaurantQuesionNonTsAssignedObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj from an XML reader. The element to be read is
 * "restaurantquesionnontsassigned".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj *_tSRestaurantQuesionNonTsAssignedObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element restaurantquesionnontsassigned."];
    }
  }

  if (xmlStrcmp(BAD_CAST "restaurantquesionnontsassigned", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}restaurantquesionnontsassigned.");
#endif
    _tSRestaurantQuesionNonTsAssignedObj = (ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj *)[ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}restaurantquesionnontsassigned.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj. Expected element restaurantquesionnontsassigned. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj. Expected element restaurantquesionnontsassigned. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj to XML under element name "restaurantquesionnontsassigned".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantquesionnontsassigned", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}restaurantquesionnontsassigned. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantQuesionNonTsAssignedObj for root element {}restaurantquesionnontsassigned...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantQuesionNonTsAssignedObj for root element {}restaurantquesionnontsassigned...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}restaurantquesionnontsassigned. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}friendsNonTsUserFacebookId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}friendsNonTsUserFacebookId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    if ([self friendsNonTsUserFacebookId]) {
      [self setFriendsNonTsUserFacebookId: [[self friendsNonTsUserFacebookId] arrayByAddingObject: __child]];
    }
    else {
      [self setFriendsNonTsUserFacebookId: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self friendsNonTsUserFacebookId]) {
    __enumerator = [[self friendsNonTsUserFacebookId] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "friendsNonTsUserFacebookId", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}friendsNonTsUserFacebookId."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}friendsNonTsUserFacebookId...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}friendsNonTsUserFacebookId...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}friendsNonTsUserFacebookId."];
      }
    } //end item iterator.
  }
}
@end /* implementation ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantQuesionNonTsAssignedObj_M */
...
{
  [newPhotoDetails retain];
  [_photoDetails release];
  _photoDetails = newPhotoDetails;
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}photoDetails of type {}TSPhotoObj.");
#endif
    __child = [ENUNCIATENS0TSPhotoObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}photoDetails of type {}TSPhotoObj.");
#endif

    [self setPhotoDetails: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self photoDetails]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}photoDetails."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}photoDetails...");
#endif
    [[self photoDetails] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}photoDetails...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}photoDetails."];
    }
  }
}
...
{
  return _restaurantsForYouObjList;
}
...
{
  [newRestaurantsForYouObjList retain];
  [_restaurantsForYouObjList release];
  _restaurantsForYouObjList = newRestaurantsForYouObjList;
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}restaurantsForYouObjList of type {}TSRestaurantsForYouObj.");
#endif

    __child = [ENUNCIATENS0TSRestaurantsForYouObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}restaurantsForYouObjList of type {}TSRestaurantsForYouObj.");
#endif

    if ([self restaurantsForYouObjList]) {
      [self setRestaurantsForYouObjList: [[self restaurantsForYouObjList] arrayByAddingObject: __child]];
    }
    else {
      [self setRestaurantsForYouObjList: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self restaurantsForYouObjList]) {
    __enumerator = [[self restaurantsForYouObjList] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantsForYouObjList", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}restaurantsForYouObjList."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}restaurantsForYouObjList...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}restaurantsForYouObjList...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}restaurantsForYouObjList."];
      }
    } //end item iterator.
  }
}
...
#ifndef DEF_ENUNCIATENS0TSRestaurantBuzzObj_M
#define DEF_ENUNCIATENS0TSRestaurantBuzzObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantBuzzObj
...
{
  [newRecommenderUser retain];
  [_recommenderUser release];
  _recommenderUser = newRecommenderUser;
}
...
{
  return _recommenderUserFolloweeFlag;
}
...
{
  [newRecommenderUserFolloweeFlag retain];
  [_recommenderUserFolloweeFlag release];
  _recommenderUserFolloweeFlag = newRecommenderUserFolloweeFlag;
}
...
{
  [newReplyDatetime retain];
  [_replyDatetime release];
  _replyDatetime = newReplyDatetime;
}
...
- (void) dealloc
{
  [self setRecorequestText: nil];
  [self setReplyText: nil];
  [self setRecommenderUser: nil];
  [self setRecommenderUserFolloweeFlag: nil];
  [self setReplyDatetime: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantBuzzObj *_eNUNCIATENS0TSRestaurantBuzzObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantBuzzObj = (ENUNCIATENS0TSRestaurantBuzzObj *) [ENUNCIATENS0TSRestaurantBuzzObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantBuzzObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantBuzzObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantBuzzObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantBuzzObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantBuzzObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantBuzzObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantBuzzObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantBuzzObj *_eNUNCIATENS0TSRestaurantBuzzObj = [[ENUNCIATENS0TSRestaurantBuzzObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantBuzzObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantBuzzObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantBuzzObj autorelease];
  return _eNUNCIATENS0TSRestaurantBuzzObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantBuzzObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantBuzzObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantBuzzObj from an XML reader. The element to be read is
 * "restsbuzz".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantBuzzObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantBuzzObj *_tSRestaurantBuzzObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element restsbuzz."];
    }
  }

  if (xmlStrcmp(BAD_CAST "restsbuzz", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}restsbuzz.");
#endif
    _tSRestaurantBuzzObj = (ENUNCIATENS0TSRestaurantBuzzObj *)[ENUNCIATENS0TSRestaurantBuzzObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}restsbuzz.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantBuzzObj. Expected element restsbuzz. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantBuzzObj. Expected element restsbuzz. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }

  return _tSRestaurantBuzzObj;
}

/**
 * Writes this ENUNCIATENS0TSRestaurantBuzzObj to XML under element name "restsbuzz".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
 * @param _tSRestaurantBuzzObj The TSRestaurantBuzzObj to write.
 * @return 1 if successful, 0 otherwise.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer
{
...
/**
 * Writes this ENUNCIATENS0TSRestaurantBuzzObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restsbuzz", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}restsbuzz. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantBuzzObj for root element {}restsbuzz...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantBuzzObj for root element {}restsbuzz...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}restsbuzz. XML writer status: %i\n", rc];
  }
}
...
- (BOOL) readJAXBChildElement: (xmlTextReaderPtr) reader
{
  ...
  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "recommenderUser", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {

#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recommenderUser of type {}TSUserProfileBasicObj.");
#endif
    __child = [ENUNCIATENS0TSUserProfileBasicObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recommenderUser of type {}TSUserProfileBasicObj.");
#endif

    [self setRecommenderUser: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recommenderUserFolloweeFlag of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recommenderUserFolloweeFlag of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRecommenderUserFolloweeFlag: __child];
    return YES;
  } //end "if choice"

  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "replyDatetime", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {

#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}replyDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}replyDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setReplyDatetime: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self recommenderUser]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommenderUser", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recommenderUser."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recommenderUser...");
#endif
    [[self recommenderUser] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recommenderUser...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recommenderUser."];
    }
  }
  if ([self recommenderUserFolloweeFlag]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommenderUserFolloweeFlag", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recommenderUserFolloweeFlag."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recommenderUserFolloweeFlag...");
#endif
    [[self recommenderUserFolloweeFlag] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recommenderUserFolloweeFlag...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recommenderUserFolloweeFlag."];
    }
  }
  if ([self replyDatetime]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "replyDatetime", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}replyDatetime."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}replyDatetime...");
#endif
    [[self replyDatetime] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}replyDatetime...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}replyDatetime."];
    }
  }
}
@end /* implementation ENUNCIATENS0TSRestaurantBuzzObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantBuzzObj_M */
#ifndef DEF_ENUNCIATENS0TSRestaurantRecommendersDetailsObj_M
#define DEF_ENUNCIATENS0TSRestaurantRecommendersDetailsObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantRecommendersDetailsObj
...
{
  return _recommendersDetailsList;
}
...
{
  [newRecommendersDetailsList retain];
  [_recommendersDetailsList release];
  _recommendersDetailsList = newRecommendersDetailsList;
}
...
- (void) dealloc
{
  [self setUserId: nil];
  [self setRestaurantId: nil];
  [self setRecommendersDetailsList: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantRecommendersDetailsObj *_eNUNCIATENS0TSRestaurantRecommendersDetailsObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantRecommendersDetailsObj = (ENUNCIATENS0TSRestaurantRecommendersDetailsObj *) [ENUNCIATENS0TSRestaurantRecommendersDetailsObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantRecommendersDetailsObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantRecommendersDetailsObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantRecommendersDetailsObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantRecommendersDetailsObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantRecommendersDetailsObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantRecommendersDetailsObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantRecommendersDetailsObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantRecommendersDetailsObj *_eNUNCIATENS0TSRestaurantRecommendersDetailsObj = [[ENUNCIATENS0TSRestaurantRecommendersDetailsObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantRecommendersDetailsObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantRecommendersDetailsObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantRecommendersDetailsObj autorelease];
  return _eNUNCIATENS0TSRestaurantRecommendersDetailsObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantRecommendersDetailsObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantRecommendersDetailsObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantRecommendersDetailsObj from an XML reader. The element to be read is
 * "restaurantrecommenders".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantRecommendersDetailsObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantRecommendersDetailsObj *_tSRestaurantRecommendersDetailsObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element restaurantrecommenders."];
    }
  }

  if (xmlStrcmp(BAD_CAST "restaurantrecommenders", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}restaurantrecommenders.");
#endif
    _tSRestaurantRecommendersDetailsObj = (ENUNCIATENS0TSRestaurantRecommendersDetailsObj *)[ENUNCIATENS0TSRestaurantRecommendersDetailsObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}restaurantrecommenders.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantRecommendersDetailsObj. Expected element restaurantrecommenders. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantRecommendersDetailsObj. Expected element restaurantrecommenders. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRestaurantRecommendersDetailsObj to XML under element name "restaurantrecommenders".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRestaurantRecommendersDetailsObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantrecommenders", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}restaurantrecommenders. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantRecommendersDetailsObj for root element {}restaurantrecommenders...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantRecommendersDetailsObj for root element {}restaurantrecommenders...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}restaurantrecommenders. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recommendersDetailsList of type {}TSUserProfileBasicObj.");
#endif

    __child = [ENUNCIATENS0TSUserProfileBasicObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recommendersDetailsList of type {}TSUserProfileBasicObj.");
#endif

    if ([self recommendersDetailsList]) {
      [self setRecommendersDetailsList: [[self recommendersDetailsList] arrayByAddingObject: __child]];
    }
    else {
      [self setRecommendersDetailsList: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self recommendersDetailsList]) {
    __enumerator = [[self recommendersDetailsList] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommendersDetailsList", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}recommendersDetailsList."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}recommendersDetailsList...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}recommendersDetailsList...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}recommendersDetailsList."];
      }
    } //end item iterator.
  }
}
@end /* implementation ENUNCIATENS0TSRestaurantRecommendersDetailsObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantRecommendersDetailsObj_M */
#ifndef DEF_ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj_M
#define DEF_ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj
...
{
  return _restaurantsSearchListTileObj;
}
...
{
  [newRestaurantsSearchListTileObj retain];
  [_restaurantsSearchListTileObj release];
  _restaurantsSearchListTileObj = newRestaurantsSearchListTileObj;
}

- (void) dealloc
{
  [self setMaxPaginationId: nil];
  [self setRestaurantsSearchListTileObj: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj *_eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj = (ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj *) [ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj *_eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj = [[ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj autorelease];
  return _eNUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj from an XML reader. The element to be read is
 * "reststilesearchextendedinfo".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj *_tSRestaurantsTileSearchExtendedInfoObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element reststilesearchextendedinfo."];
    }
  }

  if (xmlStrcmp(BAD_CAST "reststilesearchextendedinfo", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}reststilesearchextendedinfo.");
#endif
    _tSRestaurantsTileSearchExtendedInfoObj = (ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj *)[ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}reststilesearchextendedinfo.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj. Expected element reststilesearchextendedinfo. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj. Expected element reststilesearchextendedinfo. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj to XML under element name "reststilesearchextendedinfo".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "reststilesearchextendedinfo", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}reststilesearchextendedinfo. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantsTileSearchExtendedInfoObj for root element {}reststilesearchextendedinfo...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantsTileSearchExtendedInfoObj for root element {}reststilesearchextendedinfo...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}reststilesearchextendedinfo. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}restaurantsSearchListTileObj of type {}TSRestaurantsTileSearchObj.");
#endif

    __child = [ENUNCIATENS0TSRestaurantsTileSearchObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}restaurantsSearchListTileObj of type {}TSRestaurantsTileSearchObj.");
#endif

    if ([self restaurantsSearchListTileObj]) {
      [self setRestaurantsSearchListTileObj: [[self restaurantsSearchListTileObj] arrayByAddingObject: __child]];
    }
    else {
      [self setRestaurantsSearchListTileObj: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self restaurantsSearchListTileObj]) {
    __enumerator = [[self restaurantsSearchListTileObj] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantsSearchListTileObj", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}restaurantsSearchListTileObj."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}restaurantsSearchListTileObj...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}restaurantsSearchListTileObj...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}restaurantsSearchListTileObj."];
      }
    } //end item iterator.
  }
}
@end /* implementation ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantsTileSearchExtendedInfoObj_M */
...
#ifndef DEF_ENUNCIATENS0TSRestaurantsTileSearchObj_M
#define DEF_ENUNCIATENS0TSRestaurantsTileSearchObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantsTileSearchObj
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantsTileSearchObj *_eNUNCIATENS0TSRestaurantsTileSearchObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantsTileSearchObj = (ENUNCIATENS0TSRestaurantsTileSearchObj *) [ENUNCIATENS0TSRestaurantsTileSearchObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantsTileSearchObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantsTileSearchObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantsTileSearchObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantsTileSearchObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantsTileSearchObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantsTileSearchObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantsTileSearchObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantsTileSearchObj *_eNUNCIATENS0TSRestaurantsTileSearchObj = [[ENUNCIATENS0TSRestaurantsTileSearchObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantsTileSearchObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantsTileSearchObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantsTileSearchObj autorelease];
  return _eNUNCIATENS0TSRestaurantsTileSearchObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantsTileSearchObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantsTileSearchObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantsTileSearchObj from an XML reader. The element to be read is
 * "reststilesearch".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantsTileSearchObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantsTileSearchObj *_tSRestaurantsTileSearchObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element reststilesearch."];
    }
  }

  if (xmlStrcmp(BAD_CAST "reststilesearch", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}reststilesearch.");
#endif
    _tSRestaurantsTileSearchObj = (ENUNCIATENS0TSRestaurantsTileSearchObj *)[ENUNCIATENS0TSRestaurantsTileSearchObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}reststilesearch.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantsTileSearchObj. Expected element reststilesearch. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantsTileSearchObj. Expected element reststilesearch. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRestaurantsTileSearchObj to XML under element name "reststilesearch".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRestaurantsTileSearchObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "reststilesearch", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}reststilesearch. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantsTileSearchObj for root element {}reststilesearch...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantsTileSearchObj for root element {}reststilesearch...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}reststilesearch. XML writer status: %i\n", rc];
  }
}
...
@end /* implementation ENUNCIATENS0TSRestaurantsTileSearchObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantsTileSearchObj_M */
#ifndef DEF_ENUNCIATENS0TSRestaurantsForYouObj_M
#define DEF_ENUNCIATENS0TSRestaurantsForYouObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantsForYouObj
...
{
  return _recommendationsForYouList;
}
...
{
  [newRecommendationsForYouList retain];
  [_recommendationsForYouList release];
  _recommendationsForYouList = newRecommendationsForYouList;
}
...
- (void) dealloc
{
  [self setRecommendationsForYouList: nil];
  [self setRestaurantId: nil];
  [self setRestaurantName: nil];
  [self setCuisineTier2Name: nil];
  [self setPrice: nil];
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantsForYouObj *_eNUNCIATENS0TSRestaurantsForYouObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantsForYouObj = (ENUNCIATENS0TSRestaurantsForYouObj *) [ENUNCIATENS0TSRestaurantsForYouObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantsForYouObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantsForYouObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantsForYouObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantsForYouObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantsForYouObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantsForYouObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantsForYouObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantsForYouObj *_eNUNCIATENS0TSRestaurantsForYouObj = [[ENUNCIATENS0TSRestaurantsForYouObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantsForYouObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantsForYouObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantsForYouObj autorelease];
  return _eNUNCIATENS0TSRestaurantsForYouObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantsForYouObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantsForYouObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantsForYouObj from an XML reader. The element to be read is
 * "rests4u".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantsForYouObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantsForYouObj *_tSRestaurantsForYouObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element rests4u."];
    }
  }

  if (xmlStrcmp(BAD_CAST "rests4u", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}rests4u.");
#endif
    _tSRestaurantsForYouObj = (ENUNCIATENS0TSRestaurantsForYouObj *)[ENUNCIATENS0TSRestaurantsForYouObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}rests4u.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantsForYouObj. Expected element rests4u. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantsForYouObj. Expected element rests4u. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRestaurantsForYouObj to XML under element name "rests4u".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRestaurantsForYouObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "rests4u", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}rests4u. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantsForYouObj for root element {}rests4u...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantsForYouObj for root element {}rests4u...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}rests4u. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recommendationsForYouList of type {}TSRecommendationsObj.");
#endif

    __child = [ENUNCIATENS0TSRecommendationsObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recommendationsForYouList of type {}TSRecommendationsObj.");
#endif

    if ([self recommendationsForYouList]) {
      [self setRecommendationsForYouList: [[self recommendationsForYouList] arrayByAddingObject: __child]];
    }
    else {
      [self setRecommendationsForYouList: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self recommendationsForYouList]) {
    __enumerator = [[self recommendationsForYouList] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommendationsForYouList", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}recommendationsForYouList."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}recommendationsForYouList...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}recommendationsForYouList...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}recommendationsForYouList."];
      }
    } //end item iterator.
  }
  if ([self restaurantId]) {
...
@end /* implementation ENUNCIATENS0TSRestaurantsForYouObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantsForYouObj_M */
...
#ifndef DEF_ENUNCIATENS0TSRecommendationsObj_M
#define DEF_ENUNCIATENS0TSRecommendationsObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRecommendationsObj
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRecommendationsObj *_eNUNCIATENS0TSRecommendationsObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRecommendationsObj = (ENUNCIATENS0TSRecommendationsObj *) [ENUNCIATENS0TSRecommendationsObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRecommendationsObj;
}
...
@end /* implementation ENUNCIATENS0TSRecommendationsObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRecommendationsObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRecommendationsObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRecommendationsObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRecommendationsObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRecommendationsObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRecommendationsObj *_eNUNCIATENS0TSRecommendationsObj = [[ENUNCIATENS0TSRecommendationsObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRecommendationsObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRecommendationsObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRecommendationsObj autorelease];
  return _eNUNCIATENS0TSRecommendationsObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRecommendationsObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRecommendationsObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRecommendationsObj from an XML reader. The element to be read is
 * "recommendations4u".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRecommendationsObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRecommendationsObj *_tSRecommendationsObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element recommendations4u."];
    }
  }

  if (xmlStrcmp(BAD_CAST "recommendations4u", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}recommendations4u.");
#endif
    _tSRecommendationsObj = (ENUNCIATENS0TSRecommendationsObj *)[ENUNCIATENS0TSRecommendationsObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}recommendations4u.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRecommendationsObj. Expected element recommendations4u. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRecommendationsObj. Expected element recommendations4u. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRecommendationsObj to XML under element name "recommendations4u".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRecommendationsObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommendations4u", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}recommendations4u. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRecommendationsObj for root element {}recommendations4u...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRecommendationsObj for root element {}recommendations4u...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}recommendations4u. XML writer status: %i\n", rc];
  }
}
...
@end /* implementation ENUNCIATENS0TSRecommendationsObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRecommendationsObj_M */
...
{
  [newQuestionUser retain];
  [_questionUser release];
  _questionUser = newQuestionUser;
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}questionUser of type {}TSUserProfileBasicObj.");
#endif
    __child = [ENUNCIATENS0TSUserProfileBasicObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}questionUser of type {}TSUserProfileBasicObj.");
#endif

    [self setQuestionUser: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self questionUser]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "questionUser", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}questionUser."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}questionUser...");
#endif
    [[self questionUser] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}questionUser...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}questionUser."];
    }
  }
...
#ifndef DEF_ENUNCIATENS0TSRecoRequestNonAssignedObj_M
#define DEF_ENUNCIATENS0TSRecoRequestNonAssignedObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRecoRequestNonAssignedObj
...
{
  [newRecoRequestId retain];
  [_recoRequestId release];
  _recoRequestId = newRecoRequestId;
}
...
{
  return _nonassignedFacebookIdList;
}
...
{
  [newNonassignedFacebookIdList retain];
  [_nonassignedFacebookIdList release];
  _nonassignedFacebookIdList = newNonassignedFacebookIdList;
}

- (void) dealloc
{
  [self setRecoRequestId: nil];
  [self setNonassignedFacebookIdList: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRecoRequestNonAssignedObj *_eNUNCIATENS0TSRecoRequestNonAssignedObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRecoRequestNonAssignedObj = (ENUNCIATENS0TSRecoRequestNonAssignedObj *) [ENUNCIATENS0TSRecoRequestNonAssignedObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRecoRequestNonAssignedObj;
}
...
@end /* implementation ENUNCIATENS0TSRecoRequestNonAssignedObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRecoRequestNonAssignedObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRecoRequestNonAssignedObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRecoRequestNonAssignedObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRecoRequestNonAssignedObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRecoRequestNonAssignedObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRecoRequestNonAssignedObj *_eNUNCIATENS0TSRecoRequestNonAssignedObj = [[ENUNCIATENS0TSRecoRequestNonAssignedObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRecoRequestNonAssignedObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRecoRequestNonAssignedObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRecoRequestNonAssignedObj autorelease];
  return _eNUNCIATENS0TSRecoRequestNonAssignedObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRecoRequestNonAssignedObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRecoRequestNonAssignedObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRecoRequestNonAssignedObj from an XML reader. The element to be read is
 * "recorequestnonassigned".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRecoRequestNonAssignedObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRecoRequestNonAssignedObj *_tSRecoRequestNonAssignedObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element recorequestnonassigned."];
    }
  }

  if (xmlStrcmp(BAD_CAST "recorequestnonassigned", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}recorequestnonassigned.");
#endif
    _tSRecoRequestNonAssignedObj = (ENUNCIATENS0TSRecoRequestNonAssignedObj *)[ENUNCIATENS0TSRecoRequestNonAssignedObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}recorequestnonassigned.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRecoRequestNonAssignedObj. Expected element recorequestnonassigned. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRecoRequestNonAssignedObj. Expected element recorequestnonassigned. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRecoRequestNonAssignedObj to XML under element name "recorequestnonassigned".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRecoRequestNonAssignedObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recorequestnonassigned", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}recorequestnonassigned. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRecoRequestNonAssignedObj for root element {}recorequestnonassigned...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRecoRequestNonAssignedObj for root element {}recorequestnonassigned...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}recorequestnonassigned. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recoRequestId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recoRequestId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRecoRequestId: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}nonassignedFacebookIdList of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}nonassignedFacebookIdList of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    if ([self nonassignedFacebookIdList]) {
      [self setNonassignedFacebookIdList: [[self nonassignedFacebookIdList] arrayByAddingObject: __child]];
    }
    else {
      [self setNonassignedFacebookIdList: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self recoRequestId]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recoRequestId", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recoRequestId."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recoRequestId...");
#endif
    [[self recoRequestId] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recoRequestId...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recoRequestId."];
    }
  }
  if ([self nonassignedFacebookIdList]) {
    __enumerator = [[self nonassignedFacebookIdList] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "nonassignedFacebookIdList", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}nonassignedFacebookIdList."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}nonassignedFacebookIdList...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}nonassignedFacebookIdList...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}nonassignedFacebookIdList."];
      }
    } //end item iterator.
  }
}
@end /* implementation ENUNCIATENS0TSRecoRequestNonAssignedObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRecoRequestNonAssignedObj_M */
...
- (BOOL) readJAXBChildElement: (xmlTextReaderPtr) reader
{
  ...
  if (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT
    && xmlStrcmp(BAD_CAST "information", xmlTextReaderConstLocalName(reader)) == 0
    && xmlTextReaderConstNamespaceUri(reader) == NULL) {

#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}information of type {}TSRestaurantsTileSearchObj.");
#endif
    __child = [ENUNCIATENS0TSRestaurantsTileSearchObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}information of type {}TSRestaurantsTileSearchObj.");
#endif

    [self setInformation: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self information]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "information", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}information."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}information...");
#endif
    [[self information] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}information...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}information."];
    }
  }
}
...
#ifndef DEF_ENUNCIATENS0TSRestaurantBasicObj_M
#define DEF_ENUNCIATENS0TSRestaurantBasicObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSRestaurantBasicObj
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSRestaurantBasicObj *_eNUNCIATENS0TSRestaurantBasicObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSRestaurantBasicObj = (ENUNCIATENS0TSRestaurantBasicObj *) [ENUNCIATENS0TSRestaurantBasicObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSRestaurantBasicObj;
}
...
@end /* implementation ENUNCIATENS0TSRestaurantBasicObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSRestaurantBasicObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSRestaurantBasicObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSRestaurantBasicObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSRestaurantBasicObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSRestaurantBasicObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSRestaurantBasicObj *_eNUNCIATENS0TSRestaurantBasicObj = [[ENUNCIATENS0TSRestaurantBasicObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSRestaurantBasicObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSRestaurantBasicObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSRestaurantBasicObj autorelease];
  return _eNUNCIATENS0TSRestaurantBasicObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSRestaurantBasicObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSRestaurantBasicObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSRestaurantBasicObj from an XML reader. The element to be read is
 * "restaurantbasic".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSRestaurantBasicObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSRestaurantBasicObj *_tSRestaurantBasicObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element restaurantbasic."];
    }
  }

  if (xmlStrcmp(BAD_CAST "restaurantbasic", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}restaurantbasic.");
#endif
    _tSRestaurantBasicObj = (ENUNCIATENS0TSRestaurantBasicObj *)[ENUNCIATENS0TSRestaurantBasicObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}restaurantbasic.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantBasicObj. Expected element restaurantbasic. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSRestaurantBasicObj. Expected element restaurantbasic. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSRestaurantBasicObj to XML under element name "restaurantbasic".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSRestaurantBasicObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantbasic", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}restaurantbasic. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSRestaurantBasicObj for root element {}restaurantbasic...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSRestaurantBasicObj for root element {}restaurantbasic...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}restaurantbasic. XML writer status: %i\n", rc];
  }
}
...
@end /* implementation ENUNCIATENS0TSRestaurantBasicObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSRestaurantBasicObj_M */
...
#ifndef DEF_ENUNCIATENS0TSNotifRecoLikeObj_M
#define DEF_ENUNCIATENS0TSNotifRecoLikeObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifRecoLikeObj
...
{
  [newLikeDatetime retain];
  [_likeDatetime release];
  _likeDatetime = newLikeDatetime;
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifRecoLikeObj *_eNUNCIATENS0TSNotifRecoLikeObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifRecoLikeObj = (ENUNCIATENS0TSNotifRecoLikeObj *) [ENUNCIATENS0TSNotifRecoLikeObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifRecoLikeObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifRecoLikeObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifRecoLikeObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifRecoLikeObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifRecoLikeObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifRecoLikeObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifRecoLikeObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifRecoLikeObj *_eNUNCIATENS0TSNotifRecoLikeObj = [[ENUNCIATENS0TSNotifRecoLikeObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifRecoLikeObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifRecoLikeObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifRecoLikeObj autorelease];
  return _eNUNCIATENS0TSNotifRecoLikeObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifRecoLikeObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifRecoLikeObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifRecoLikeObj from an XML reader. The element to be read is
 * "notifrecolike".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifRecoLikeObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifRecoLikeObj *_tSNotifRecoLikeObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notifrecolike."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notifrecolike", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notifrecolike.");
#endif
    _tSNotifRecoLikeObj = (ENUNCIATENS0TSNotifRecoLikeObj *)[ENUNCIATENS0TSNotifRecoLikeObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notifrecolike.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecoLikeObj. Expected element notifrecolike. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecoLikeObj. Expected element notifrecolike. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifRecoLikeObj to XML under element name "notifrecolike".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifRecoLikeObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notifrecolike", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notifrecolike. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifRecoLikeObj for root element {}notifrecolike...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifRecoLikeObj for root element {}notifrecolike...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notifrecolike. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}likeId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}likeId of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setLikeId: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}likeDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}likeDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setLikeDatetime: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self likeId]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}likeId."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}likeId...");
#endif
    [[self likeId] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}likeId...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}likeId."];
    }
  }
  if ([self likeDatetime]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "likeDatetime", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}likeDatetime."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}likeDatetime...");
#endif
    [[self likeDatetime] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}likeDatetime...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}likeDatetime."];
    }
  }
  if ([self viewed]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "viewed", NULL);
...
@end /* implementation ENUNCIATENS0TSNotifRecoLikeObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifRecoLikeObj_M */
#ifndef DEF_ENUNCIATENS0TSNotifRecorequestNeededObj_M
#define DEF_ENUNCIATENS0TSNotifRecorequestNeededObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifRecorequestNeededObj
...
{
  [newAssignedDatetime retain];
  [_assignedDatetime release];
  _assignedDatetime = newAssignedDatetime;
}
...
{
  return _recorequestassignedViewed;
}
...
{
  [newRecorequestassignedViewed retain];
  [_recorequestassignedViewed release];
  _recorequestassignedViewed = newRecorequestassignedViewed;
}
...
{
  return _recorequestassignedActioned;
}
...
{
  [newRecorequestassignedActioned retain];
  [_recorequestassignedActioned release];
  _recorequestassignedActioned = newRecorequestassignedActioned;
}
...
{
  [self setRecorequestId: nil];
  [self setAssignedDatetime: nil];
  [self setRecorequestassignedViewed: nil];
  [self setRecorequestassignedActioned: nil];
  [self setRecommendeeUser: nil];
  [self setRecorequestText: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifRecorequestNeededObj *_eNUNCIATENS0TSNotifRecorequestNeededObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifRecorequestNeededObj = (ENUNCIATENS0TSNotifRecorequestNeededObj *) [ENUNCIATENS0TSNotifRecorequestNeededObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifRecorequestNeededObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifRecorequestNeededObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifRecorequestNeededObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifRecorequestNeededObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifRecorequestNeededObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifRecorequestNeededObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifRecorequestNeededObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifRecorequestNeededObj *_eNUNCIATENS0TSNotifRecorequestNeededObj = [[ENUNCIATENS0TSNotifRecorequestNeededObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifRecorequestNeededObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifRecorequestNeededObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifRecorequestNeededObj autorelease];
  return _eNUNCIATENS0TSNotifRecorequestNeededObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifRecorequestNeededObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifRecorequestNeededObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifRecorequestNeededObj from an XML reader. The element to be read is
 * "notifrecorequestneeded".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifRecorequestNeededObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifRecorequestNeededObj *_tSNotifRecorequestNeededObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notifrecorequestneeded."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notifrecorequestneeded", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notifrecorequestneeded.");
#endif
    _tSNotifRecorequestNeededObj = (ENUNCIATENS0TSNotifRecorequestNeededObj *)[ENUNCIATENS0TSNotifRecorequestNeededObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notifrecorequestneeded.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecorequestNeededObj. Expected element notifrecorequestneeded. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecorequestNeededObj. Expected element notifrecorequestneeded. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifRecorequestNeededObj to XML under element name "notifrecorequestneeded".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifRecorequestNeededObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notifrecorequestneeded", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notifrecorequestneeded. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifRecorequestNeededObj for root element {}notifrecorequestneeded...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifRecorequestNeededObj for root element {}notifrecorequestneeded...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notifrecorequestneeded. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}assignedDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}assignedDatetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setAssignedDatetime: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recorequestassignedViewed of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recorequestassignedViewed of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRecorequestassignedViewed: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recorequestassignedActioned of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recorequestassignedActioned of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRecorequestassignedActioned: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self assignedDatetime]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "assignedDatetime", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}assignedDatetime."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}assignedDatetime...");
#endif
    [[self assignedDatetime] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}assignedDatetime...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}assignedDatetime."];
    }
  }
  if ([self recorequestassignedViewed]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recorequestassignedViewed", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recorequestassignedViewed."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recorequestassignedViewed...");
#endif
    [[self recorequestassignedViewed] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recorequestassignedViewed...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recorequestassignedViewed."];
    }
  }
  if ([self recorequestassignedActioned]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recorequestassignedActioned", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recorequestassignedActioned."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recorequestassignedActioned...");
#endif
    [[self recorequestassignedActioned] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recorequestassignedActioned...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recorequestassignedActioned."];
    }
  }
  if ([self recommendeeUser]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommendeeUser", NULL);
...
@end /* implementation ENUNCIATENS0TSNotifRecorequestNeededObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifRecorequestNeededObj_M */
#ifndef DEF_ENUNCIATENS0TSNotifRecorequestAnswerObj_M
#define DEF_ENUNCIATENS0TSNotifRecorequestAnswerObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifRecorequestAnswerObj
...
{
  [newMinViewedIndicator retain];
  [_minViewedIndicator release];
  _minViewedIndicator = newMinViewedIndicator;
}
...
{
  [newRecoActioned retain];
  [_recoActioned release];
  _recoActioned = newRecoActioned;
}
...
{
  [newRecommendationsForYou retain];
  [_recommendationsForYou release];
  _recommendationsForYou = newRecommendationsForYou;
}

- (void) dealloc
{
  [self setRecorequestId: nil];
  [self setMinViewedIndicator: nil];
  [self setRecoActioned: nil];
  [self setRecoReply: nil];
  [self setRecommendationsForYou: nil];
  [super dealloc];
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifRecorequestAnswerObj *_eNUNCIATENS0TSNotifRecorequestAnswerObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifRecorequestAnswerObj = (ENUNCIATENS0TSNotifRecorequestAnswerObj *) [ENUNCIATENS0TSNotifRecorequestAnswerObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifRecorequestAnswerObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifRecorequestAnswerObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifRecorequestAnswerObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifRecorequestAnswerObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifRecorequestAnswerObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifRecorequestAnswerObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifRecorequestAnswerObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifRecorequestAnswerObj *_eNUNCIATENS0TSNotifRecorequestAnswerObj = [[ENUNCIATENS0TSNotifRecorequestAnswerObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifRecorequestAnswerObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifRecorequestAnswerObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifRecorequestAnswerObj autorelease];
  return _eNUNCIATENS0TSNotifRecorequestAnswerObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifRecorequestAnswerObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifRecorequestAnswerObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifRecorequestAnswerObj from an XML reader. The element to be read is
 * "notifrecorequestanswer".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifRecorequestAnswerObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifRecorequestAnswerObj *_tSNotifRecorequestAnswerObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notifrecorequestanswer."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notifrecorequestanswer", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notifrecorequestanswer.");
#endif
    _tSNotifRecorequestAnswerObj = (ENUNCIATENS0TSNotifRecorequestAnswerObj *)[ENUNCIATENS0TSNotifRecorequestAnswerObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notifrecorequestanswer.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecorequestAnswerObj. Expected element notifrecorequestanswer. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifRecorequestAnswerObj. Expected element notifrecorequestanswer. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifRecorequestAnswerObj to XML under element name "notifrecorequestanswer".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifRecorequestAnswerObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notifrecorequestanswer", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notifrecorequestanswer. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifRecorequestAnswerObj for root element {}notifrecorequestanswer...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifRecorequestAnswerObj for root element {}notifrecorequestanswer...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notifrecorequestanswer. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}minViewedIndicator of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}minViewedIndicator of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setMinViewedIndicator: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recoActioned of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recoActioned of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setRecoActioned: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recoReply of type {}TSNotifRecoReplyObj.");
#endif

    __child = [ENUNCIATENS0TSNotifRecoReplyObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recoReply of type {}TSNotifRecoReplyObj.");
#endif

    if ([self recoReply]) {
      [self setRecoReply: [[self recoReply] arrayByAddingObject: __child]];
    }
    else {
      [self setRecoReply: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}recommendationsForYou of type {}TSRecommendationsForYouObj.");
#endif
    __child = [ENUNCIATENS0TSRecommendationsForYouObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}recommendationsForYou of type {}TSRecommendationsForYouObj.");
#endif

    [self setRecommendationsForYou: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self minViewedIndicator]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "minViewedIndicator", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}minViewedIndicator."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}minViewedIndicator...");
#endif
    [[self minViewedIndicator] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}minViewedIndicator...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}minViewedIndicator."];
    }
  }
  if ([self recoActioned]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recoActioned", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recoActioned."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recoActioned...");
#endif
    [[self recoActioned] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recoActioned...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recoActioned."];
    }
  }
  if ([self recoReply]) {
    __enumerator = [[self recoReply] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recoReply", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}recoReply."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}recoReply...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}recoReply...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}recoReply."];
      }
    } //end item iterator.
  }
  if ([self recommendationsForYou]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "recommendationsForYou", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}recommendationsForYou."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}recommendationsForYou...");
#endif
    [[self recommendationsForYou] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}recommendationsForYou...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}recommendationsForYou."];
    }
  }
}
@end /* implementation ENUNCIATENS0TSNotifRecorequestAnswerObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifRecorequestAnswerObj_M */
#ifndef DEF_ENUNCIATENS0TSNotifFollowupQuestionObj_M
#define DEF_ENUNCIATENS0TSNotifFollowupQuestionObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifFollowupQuestionObj
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifFollowupQuestionObj *_eNUNCIATENS0TSNotifFollowupQuestionObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifFollowupQuestionObj = (ENUNCIATENS0TSNotifFollowupQuestionObj *) [ENUNCIATENS0TSNotifFollowupQuestionObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifFollowupQuestionObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifFollowupQuestionObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifFollowupQuestionObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifFollowupQuestionObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifFollowupQuestionObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifFollowupQuestionObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifFollowupQuestionObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifFollowupQuestionObj *_eNUNCIATENS0TSNotifFollowupQuestionObj = [[ENUNCIATENS0TSNotifFollowupQuestionObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifFollowupQuestionObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifFollowupQuestionObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifFollowupQuestionObj autorelease];
  return _eNUNCIATENS0TSNotifFollowupQuestionObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifFollowupQuestionObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifFollowupQuestionObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifFollowupQuestionObj from an XML reader. The element to be read is
 * "notiffollowupquestion".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifFollowupQuestionObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifFollowupQuestionObj *_tSNotifFollowupQuestionObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notiffollowupquestion."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notiffollowupquestion", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notiffollowupquestion.");
#endif
    _tSNotifFollowupQuestionObj = (ENUNCIATENS0TSNotifFollowupQuestionObj *)[ENUNCIATENS0TSNotifFollowupQuestionObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notiffollowupquestion.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifFollowupQuestionObj. Expected element notiffollowupquestion. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifFollowupQuestionObj. Expected element notiffollowupquestion. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifFollowupQuestionObj to XML under element name "notiffollowupquestion".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifFollowupQuestionObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notiffollowupquestion", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notiffollowupquestion. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifFollowupQuestionObj for root element {}notiffollowupquestion...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifFollowupQuestionObj for root element {}notiffollowupquestion...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notiffollowupquestion. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}actioned of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}actioned of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setActioned: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self actioned]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}actioned."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}actioned...");
#endif
    [[self actioned] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}actioned...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}actioned."];
    }
  }
  if ([self questionUser]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "questionUser", NULL);
...
@end /* implementation ENUNCIATENS0TSNotifFollowupQuestionObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifFollowupQuestionObj_M */
#ifndef DEF_ENUNCIATENS0TSNotifMessageForYouObj_M
#define DEF_ENUNCIATENS0TSNotifMessageForYouObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifMessageForYouObj
...
{
  [newMessageCreatedTime retain];
  [_messageCreatedTime release];
  _messageCreatedTime = newMessageCreatedTime;
}
...
- (void) dealloc
{
  ...
  [self setMessageCreatedTime: nil];
  [self setViewed: nil];
  [self setActioned: nil];
  [self setSenderUser: nil];
  [self setMessage: nil];
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifMessageForYouObj *_eNUNCIATENS0TSNotifMessageForYouObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifMessageForYouObj = (ENUNCIATENS0TSNotifMessageForYouObj *) [ENUNCIATENS0TSNotifMessageForYouObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifMessageForYouObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifMessageForYouObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifMessageForYouObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifMessageForYouObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifMessageForYouObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifMessageForYouObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifMessageForYouObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifMessageForYouObj *_eNUNCIATENS0TSNotifMessageForYouObj = [[ENUNCIATENS0TSNotifMessageForYouObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifMessageForYouObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifMessageForYouObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifMessageForYouObj autorelease];
  return _eNUNCIATENS0TSNotifMessageForYouObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifMessageForYouObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifMessageForYouObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifMessageForYouObj from an XML reader. The element to be read is
 * "notifmessageforyou".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifMessageForYouObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifMessageForYouObj *_tSNotifMessageForYouObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notifmessageforyou."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notifmessageforyou", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notifmessageforyou.");
#endif
    _tSNotifMessageForYouObj = (ENUNCIATENS0TSNotifMessageForYouObj *)[ENUNCIATENS0TSNotifMessageForYouObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notifmessageforyou.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifMessageForYouObj. Expected element notifmessageforyou. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifMessageForYouObj. Expected element notifmessageforyou. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifMessageForYouObj to XML under element name "notifmessageforyou".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifMessageForYouObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notifmessageforyou", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notifmessageforyou. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifMessageForYouObj for root element {}notifmessageforyou...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifMessageForYouObj for root element {}notifmessageforyou...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notifmessageforyou. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}messageCreatedTime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}messageCreatedTime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setMessageCreatedTime: __child];
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self messageCreatedTime]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "messageCreatedTime", NULL);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}messageCreatedTime."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}messageCreatedTime...");
#endif
    [[self messageCreatedTime] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}messageCreatedTime...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}messageCreatedTime."];
    }
  }
  if ([self viewed]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "viewed", NULL);
...
@end /* implementation ENUNCIATENS0TSNotifMessageForYouObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifMessageForYouObj_M */
#ifndef DEF_ENUNCIATENS0TSNotifDidYouLikeObj_M
#define DEF_ENUNCIATENS0TSNotifDidYouLikeObj_M

/**
 * (no documentation provided)
 */
@implementation ENUNCIATENS0TSNotifDidYouLikeObj
...
{
  [newRestaurantbasic retain];
  [_restaurantbasic release];
  _restaurantbasic = newRestaurantbasic;
}
...
+ (id<EnunciateXML>) readFromXML: (NSData *) xml
{
  ENUNCIATENS0TSNotifDidYouLikeObj *_eNUNCIATENS0TSNotifDidYouLikeObj;
  xmlTextReaderPtr reader = xmlReaderForMemory([xml bytes], [xml length], NULL, NULL, 0);
  if (reader == NULL) {
    [NSException raise: @"XMLReadError"
                 format: @"Error instantiating an XML reader."];
  ...
  _eNUNCIATENS0TSNotifDidYouLikeObj = (ENUNCIATENS0TSNotifDidYouLikeObj *) [ENUNCIATENS0TSNotifDidYouLikeObj readXMLElement: reader];
  xmlFreeTextReader(reader); //free the reader
  return _eNUNCIATENS0TSNotifDidYouLikeObj;
}
...
@end /* implementation ENUNCIATENS0TSNotifDidYouLikeObj */

/**
 * Internal, private interface for JAXB reading and writing.
 */
@interface ENUNCIATENS0TSNotifDidYouLikeObj (JAXB) <JAXBReading, JAXBWriting, JAXBType, JAXBElement>

@end /*interface ENUNCIATENS0TSNotifDidYouLikeObj (JAXB)*/

/**
 * Internal, private implementation for JAXB reading and writing.
 */
@implementation ENUNCIATENS0TSNotifDidYouLikeObj (JAXB)

/**
 * Read an instance of ENUNCIATENS0TSNotifDidYouLikeObj from an XML reader.
 *
 * @param reader The reader.
 * @return An instance of ENUNCIATENS0TSNotifDidYouLikeObj defined by the XML reader.
 */
+ (id<JAXBType>) readXMLType: (xmlTextReaderPtr) reader
{
  ENUNCIATENS0TSNotifDidYouLikeObj *_eNUNCIATENS0TSNotifDidYouLikeObj = [[ENUNCIATENS0TSNotifDidYouLikeObj alloc] init];
  NS_DURING
  {
    [_eNUNCIATENS0TSNotifDidYouLikeObj initWithReader: reader];
  }
  NS_HANDLER
  {
    _eNUNCIATENS0TSNotifDidYouLikeObj = nil;
    [localException raise];
  }
  NS_ENDHANDLER

  [_eNUNCIATENS0TSNotifDidYouLikeObj autorelease];
  return _eNUNCIATENS0TSNotifDidYouLikeObj;
}

/**
 * Initialize this instance of ENUNCIATENS0TSNotifDidYouLikeObj according to
 * the XML being read from the reader.
 *
 * @param reader The reader.
 */
...
/**
 * Write the XML for this instance of ENUNCIATENS0TSNotifDidYouLikeObj to the writer.
 * Note that since we're only writing the XML type,
 * No start/end element will be written.
 *
 * @param reader The reader.
...
/**
 * Reads a ENUNCIATENS0TSNotifDidYouLikeObj from an XML reader. The element to be read is
 * "notifdidyoulike".
 *
 * @param reader The XML reader.
 * @return The ENUNCIATENS0TSNotifDidYouLikeObj.
 */
+ (id<JAXBElement>) readXMLElement: (xmlTextReaderPtr) reader {
  int status;
  ENUNCIATENS0TSNotifDidYouLikeObj *_tSNotifDidYouLikeObj = nil;

  if (xmlTextReaderNodeType(reader) != XML_READER_TYPE_ELEMENT) {
    status = xmlTextReaderAdvanceToNextStartOrEndElement(reader);
    if (status < 1) {
      [NSException raise: @"XMLReadError"
                   format: @"Error advancing the reader to start element notifdidyoulike."];
    }
  }

  if (xmlStrcmp(BAD_CAST "notifdidyoulike", xmlTextReaderConstLocalName(reader)) == 0
      && xmlTextReaderConstNamespaceUri(reader) == NULL) {
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read root element {}notifdidyoulike.");
#endif
    _tSNotifDidYouLikeObj = (ENUNCIATENS0TSNotifDidYouLikeObj *)[ENUNCIATENS0TSNotifDidYouLikeObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Successfully read root element {}notifdidyoulike.");
#endif
  }
  else {
    if (xmlTextReaderConstNamespaceUri(reader) == NULL) {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifDidYouLikeObj. Expected element notifdidyoulike. Current element: {}%s", xmlTextReaderConstLocalName(reader)];
    }
    else {
      [NSException raise: @"XMLReadError"
                   format: @"Unable to read ENUNCIATENS0TSNotifDidYouLikeObj. Expected element notifdidyoulike. Current element: {%s}%s\n", xmlTextReaderConstNamespaceUri(reader), xmlTextReaderConstLocalName(reader)];
    }
  }
...
/**
 * Writes this ENUNCIATENS0TSNotifDidYouLikeObj to XML under element name "notifdidyoulike".
 * The namespace declarations for the element will be written.
 *
 * @param writer The XML writer.
...
/**
 * Writes this ENUNCIATENS0TSNotifDidYouLikeObj to an XML writer.
 *
 * @param writer The writer.
 * @param writeNs Whether to write the namespaces for this element to the xml writer.
 */
- (void) writeXMLElement: (xmlTextWriterPtr) writer writeNamespaces: (BOOL) writeNs
{
  int rc = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "notifdidyoulike", NULL);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing start element {}notifdidyoulike. XML writer status: %i\n", rc];
  }

#if DEBUG_ENUNCIATE > 1
  NSLog(@"writing type {}TSNotifDidYouLikeObj for root element {}notifdidyoulike...");
#endif
  [self writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
  NSLog(@"successfully wrote type {}TSNotifDidYouLikeObj for root element {}notifdidyoulike...");
#endif
  rc = xmlTextWriterEndElement(writer);
  if (rc < 0) {
    [NSException raise: @"XMLWriteError"
                 format: @"Error writing end element {}notifdidyoulike. XML writer status: %i\n", rc];
  }
}
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}datetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif
    __child = [NSString readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}datetime of type {http://www.w3.org/2001/XMLSchema}string.");
#endif

    [self setDatetime: __child];
    return YES;
  } //end "if choice"
...
#if DEBUG_ENUNCIATE > 1
    NSLog(@"Attempting to read choice {}restaurantbasic of type {}TSRestaurantBasicObj.");
#endif

    __child = [ENUNCIATENS0TSRestaurantBasicObj readXMLType: reader];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully read choice {}restaurantbasic of type {}TSRestaurantBasicObj.");
#endif

    if ([self restaurantbasic]) {
      [self setRestaurantbasic: [[self restaurantbasic] arrayByAddingObject: __child]];
    }
    else {
      [self setRestaurantbasic: [NSArray arrayWithObject: __child]];
    }
    return YES;
  } //end "if choice"
...
- (void) writeJAXBChildElements: (xmlTextWriterPtr) writer
{
  ...
  if ([self datetime]) {
    ...
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing start child element {}datetime."];
    }

#if DEBUG_ENUNCIATE > 1
    NSLog(@"writing element {}datetime...");
#endif
    [[self datetime] writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
    NSLog(@"successfully wrote element {}datetime...");
#endif

    status = xmlTextWriterEndElement(writer);
    if (status < 0) {
      [NSException raise: @"XMLWriteError"
                   format: @"Error writing end child element {}datetime."];
    }
  }
  if ([self viewed]) {
    status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "viewed", NULL);
  ...
  if ([self restaurantbasic]) {
    __enumerator = [[self restaurantbasic] objectEnumerator];

    while ( (__item = [__enumerator nextObject]) ) {
      status = xmlTextWriterStartElementNS(writer, NULL, BAD_CAST "restaurantbasic", NULL);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing start child element {}restaurantbasic."];
      }

#if DEBUG_ENUNCIATE > 1
      NSLog(@"writing element {}restaurantbasic...");
#endif
      [__item writeXMLType: writer];
#if DEBUG_ENUNCIATE > 1
      NSLog(@"successfully wrote element {}restaurantbasic...");
#endif

      status = xmlTextWriterEndElement(writer);
      if (status < 0) {
        [NSException raise: @"XMLWriteError"
                     format: @"Error writing end child element {}restaurantbasic."];
      }
    } //end item iterator.
  }
}
@end /* implementation ENUNCIATENS0TSNotifDidYouLikeObj (JAXB) */

#endif /* DEF_ENUNCIATENS0TSNotifDidYouLikeObj_M */
