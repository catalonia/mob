...
//    NSLog(@"actualSize %i", actualSize);
    ...
    }else
    {
        ...
        photoObj.image = [photoObj.image stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
        NSLog(@"download %@%@", LINK_IMAGE_QUESTION, photoObj.image);
        NSString *strURL = [NSString stringWithFormat:@"%@/%@", LINK_IMAGE_QUESTION, photoObj.image];
        DataLoader *_dataLoader = [[[DataLoader alloc] init] autorelease];
        [_dataLoader loadDataWithStringURL:strURL delegate:self withKey:[NSString stringWithFormat:@"%d", index]];
...
//        RestaurantObj *photoObj = [arrayObjects objectAtIndex:index + 1];
        ...
        if(image)
        {
            ...
            NSString *strURL = [NSString stringWithFormat:@"%@/%@", LINK_IMAGE_QUESTION, photoObj.image];
            DataLoader *_dataLoader = [[[DataLoader alloc] init] autorelease];
            [_dataLoader loadDataWithStringURL:strURL delegate:self withKey:[NSString stringWithFormat:@"%d", index + 1]];
*/
        }
