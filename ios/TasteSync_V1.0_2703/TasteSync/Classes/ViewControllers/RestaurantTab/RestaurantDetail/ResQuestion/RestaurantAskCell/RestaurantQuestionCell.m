...
{
    if (check) {
        [CommonHelpers setBackgroundImage:[UIImage imageNamed:@"ic_check.png"] forButton:self.ticker];
        check = NO;
        [self.delegate checkStatus:self.userObj checked:NO];
    }
    else {
        [CommonHelpers setBackgroundImage:[UIImage imageNamed:@"ic_check_on.png"] forButton:self.ticker];
        check = YES;
